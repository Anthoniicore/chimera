...
namespace Chimera {
    ...
    static void copy_objects() noexcept {
        ...
        for(std::size_t i = 0; i < OBJECT_BUFFER_SIZE; i++) {
            ...
            float displacement_sq = distance_squared(current_tick_object.center, previous_tick[i].center);
            current_tick_object.interpolate = displacement_sq < MAX_INTERPOLATION_DISTANCES[object->object.type == OBJECT_TYPE_BIPED];

            // Extra check, same idea as the local first person camera (see
            // camera.cpp): a moderate jump combined with a velocity that
            // doesn't remotely explain it is very likely a network
            // correction/rubber-band rather than real movement. Smoothly
            // interpolating through a position the object never truly
            // occupied is exactly what makes shots that look like they
            // land actually miss - the true (collision) position has
            // already snapped by the time the blended frame is drawn.
            // Catch this even when it falls under the gross threshold
            // above, which only catches obvious teleports.
            if(current_tick_object.interpolate) {
                ...
                bool moved_noticeably = displacement_sq > VELOCITY_MISMATCH_DISTANCE_SQ;
                bool velocity_too_low = magnitude_squared3d(object->object.translational_velocity) <= VELOCITY_MISMATCH_VELOCITY_SQ;

                if(moved_noticeably && velocity_too_low) {
                    current_tick_object.interpolate = false;
                }
            }
