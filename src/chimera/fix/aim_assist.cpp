...
extern "C" {
    ...
    float aim_assist_tick_scale = 30.0F;
...
namespace Chimera {
    bool aim_assist_command(int argc, const char **argv) {
        ...
        if(argc == 1) {
            ...
            if(strength_value) {
                ...
                aim_assist_tick_scale = 30.0F * strength;
            }
            else {
    ...
    void set_up_aim_assist_fix() noexcept {
        ...
        // Keep Halo's native final scaling instruction intact and only replace
        // its absolute multiplier address. The native instruction is:
        //   fmul dword ptr [0x612190]
        // Using our DLL-owned float preserves all native target selection,
        // visibility/team checks, angle calculation and tick compensation.
        const auto scale_address = reinterpret_cast<std::uint32_t>(&aim_assist_tick_scale);
        overwrite(aim_assist + 0x359, scale_address);
